...
### 🎯 Pattern Priority Guide 模式优先级指南

> 不是所有模式都同等重要！以下按考试频率和实际使用频率分层。

**⭐⭐⭐ Tier 1 — Must Know（必须掌握，考试高频）**

| Pattern | 现实类比 | 一句话理解 |
|---------|---------|-----------|
| **Strategy** | 导航 App 切换路线（最快/最短/避高速） | 把算法装进"盒子"，随时换一个 |
| **Observer** | 微信群消息：一人发，所有人收到通知 | 一对多通知机制 |
| **Factory Method** | 不同国家的 Pizza 店用同一流程但不同配方 | 子类决定创建什么对象 |
| **Singleton** | 一个国家只有一个总统 | 全局唯一实例 |

**⭐⭐ Tier 2 — Should Know（应该理解，考试可能出）**

| Pattern | 现实类比 | 一句话理解 |
|---------|---------|-----------|
| **Decorator** | 给手机贴膜、加壳、挂绳 — 层层装饰 | 动态叠加功能而不改类 |
| **Adapter** | 出国旅游用电源转换头 | 让不兼容接口能一起工作 |
| **Facade** | 酒店前台 — 一个电话搞定所有服务 | 简化复杂子系统的统一入口 |
| **Template Method** | 做菜的固定流程：洗菜→切菜→炒菜→装盘，但每道菜的"炒"不同 | 算法骨架固定，具体步骤子类实现 |
| **Command** | 餐厅点单：服务员（Invoker）把订单（Command）交给厨师（Receiver） | 请求封装成对象，支持撤销 |

**⭐ Tier 3 — Good to Know（了解即可）**

| Pattern | 现实类比 | 一句话理解 |
|---------|---------|-----------|
| **Composite** | 文件夹套文件夹套文件 — 统一操作 | 树形结构，叶子和组合统一对待 |
| **State** | 自动售货机：投币→选择→出货→找零，每个状态响应不同 | 对象行为随状态自动切换 |
| **Iterator** | 遥控器的"下一首"按钮 — 不用管播放列表怎么存的 | 统一遍历接口 |
| **Abstract Factory** | 宜家的北欧风/现代风/中式风系列 — 整套搭配 | 创建一族相关产品 |
...
> 🎯 **现实类比**: 导航 App 选择路线策略 — 你可以随时从"最快路线"切换到"避免高速"，App 本身不需要改变，只需要换一个路线算法。
...
> 🎯 **现实类比**: 微信群聊 — 群主（Subject）发一条消息，所有群成员（Observers）都收到通知。新成员可以随时加入，退群也不影响其他人。
...
> 🎯 **现实类比**: 给手机加配件 — 先买裸机（Component），再加手机壳（Decorator 1），再贴膜（Decorator 2），再挂绳（Decorator 3）。每层装饰都是可选的、可叠加的。
...
> 🎯 **现实类比**: 出国旅游的电源转换头 — 你的中国插头（Adaptee）不能直接插入美国插座（Target），转换头（Adapter）让它们兼容。
...
> 🎯 **现实类比**: 酒店前台 — 你只需打一个电话："帮我订车、叫早、送餐"，前台（Facade）协调内部所有部门（子系统），你不需要分别联系司机、前台、厨房。
...
> 🎯 **现实类比**: 餐厅点单 — 服务员（Invoker）把订单纸（Command）交给厨师（Receiver）。订单可以排队、取消（undo）、记录（logging）。
...
> 🎯 **现实类比**: 做菜的标准流程 — 步骤固定（洗菜→切菜→炒菜→装盘），但每道菜的"炒"和"调味"步骤不同。基类定义流程，子类实现具体步骤。
...
> 🎯 **现实类比**: 自动售货机 — 待投币状态下按按钮无反应；已投币状态下按按钮才出货。同一个按钮，行为随状态自动切换。
...
> 📌 **Test Doubles 的详细讲解（五种类型 + Python unittest.mock + Mockito + 完整代码示例）见 [[Week6-TestDoubles-Methodologies]]。** 这里只做快速回顾。

**核心概念**: 当被测对象依赖外部系统（数据库、API）时，用 Test Double 替代真实依赖，使测试更快、更可控。通过 [[Week3-Reviews-OODesign-Refactoring#5. Additional OO Design Principles 补充设计原则|Dependency Injection]] 注入。

**五种类型速记** (复杂度递增):

```
Dummy → Stub → Spy → Mock → Fake
填参数    返固定值   记录调用   验证交互   简化实现
```

| Type | 现实类比 | 一句话 |
|------|---------|--------|
| **Dummy** | 电影里的假道具 | 占位用，从不被实际使用 |
| **Stub** | 录音机自动回复 | 对特定问题返回固定答案 |
| **Spy** | 装了监控的录音机 | 回答问题 + 记录谁问了什么 |
| **Mock** | 考官 | 预设"应该被问到这些问题"，验证是否真的被问到 |
| **Fake** | 模拟驾驶器 | 有真实功能但更简单（如内存数据库替代真数据库） |
...
|---------|-----------|
| **V-Model** | 开发↔测试一一对应 |
| **Test Doubles** | Dummy → Stub → Spy → Mock → Fake (详见 [[Week6-TestDoubles-Methodologies]]) |
